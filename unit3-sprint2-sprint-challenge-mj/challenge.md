...
The relationship between the Employee and Territory tables is 'One to Many.' Each territory id is assigned to 1 specific employee but the many comes into play when certain employees can be assigned to more than 1 territory. An example of this would be Robert King. He has 10 different territories, which happens to be the most in the company. 

- What is a situation where a document store (like MongoDB) is appropriate, and
  what is a situation where it is not appropriate?

MongoDB would be appropriate in a situation where unstructured databases are needed for the user due to the nature of their data or the size (Big Data). Because it's unstructured it doesn't have a schema, which may be beneficial to users who are figuring out the organization for their data management but don't want to commit to anything just yet. MongoDB has a lot of flexibility that will allow for this sort of thing. A situation where it may not be important is if user requires a higher standard of security like ACID guarantees. A good example would be financial institutions who process countless transactions. If anything is disrupted or fails, in MondoDB it could be catastrophic without that structure in place that a PostgreSql database could provide.
- What is "NewSQL", and what is it trying to achieve?

In a nutshell, NewSQL tries to combine the best features all the different types
of SQL databases achieve. It has ACID guarantees, which provide assurance for users that data will be handled correctly, securely and consistently and will not run the risk of being lost. It also as given in the name, contains the SQL query language which is streamlined, easy to adapt and fairly universal. And perhaps the most hybrid feature is it has a relational database structure that also allows for horizontal scalability. This scalability is what draws users to NoSQL but NewSQL allows for this while also providing the structure of SQL databases.
